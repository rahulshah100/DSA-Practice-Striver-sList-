...
# SC: O(1) Explanation: Only two new pointers as used at any given point during the execution of the algorithm, which are headBB and headAA. As both pointers are using constant O(1) extra space, we've O(1) space complexity in total.
...
class Solution:
    def getIntersectionNode(self, headA: ListNode, headB: ListNode) -> Optional[ListNode]:
        ...
        while headB:
            ...
            headB=headB.next
        return None
# TC: >O(n+m) Explanation: Traversing n nodes to store linked list 1 in the hashmap and traversing m nodes to further check for entire linked list 2 if any of the nodes are present in the hashmap, will make us have n+m TC. Also the checking in hashmap i.e. 'in' wont likely work in constant space here, as the items stored in our list type hashmap are objects, and so hashing them might be complicated resulting in more than O(1) TC, which will add onto m+n Time and make resulting TC to be more than m+n.
...
# Approach 3: Using Dict hashmap. Iterate through any one linked list and store the address of all of it's nodes in a hashmap. Further, traverse over the remaining linked list and check for all of it's node, if their address is found in hashmap. If so, return the node on that address.
class Solution:
    def getIntersectionNode(self, headA: ListNode, headB: ListNode) -> Optional[ListNode]:
        hashmap={}
        while headA:
        ...
        while headB:
            ...
            headB=headB.next
        return None
# TC: O(n+m) Explanation: Here, we are making the keys as simple characters, unlike objects in Appraoch2, and so hashing/finding the item in hashmap will happen in O(1) TC.
# SC: O(n)

# Approach 4: Considering the possibility of linked lists having different lengths, we'll firstly find their length and will subtract the difference. We'll make the longer linked list's head come forward by the difference. From here on, we'll move heads of both the linked lists and will keep checking if they are pointing at the same location, in which case we return the head. If not, we return None.
# Definition for singly-linked list.
# class ListNode:
#     def __init__(self, x):
#         self.val = x
...
class Solution:
    def getIntersectionNode(self, headA: ListNode, headB: ListNode) -> Optional[ListNode]:
        ...
        return headAA
# TC: O(2M) Explanation: Assuming M is length of longer list or even equal to N, we'll have time complexity computed in the same way as it was in approach4, and we will get total TC as 2M.
# SC: O(1)