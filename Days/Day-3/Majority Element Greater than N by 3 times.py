...
class Solution:
    def majorityElement(self, nums: List[int]) -> List[int]:
        ...
        counter2 = 0
        for i in range(len(nums)):
            if counter1 == 0 and candidate2 != nums[i]:
                    candidate1 = nums[i]
            elif counter2 == 0 and candidate1 != nums[i]:
                    candidate2 = nums[i]
...
# Time Complexity: O(2n). Explaination: O(n) for traversal to get candidate1 and 2, and further O(n) to check count of candidate1 and 2. In total:2n; We can generalize it as O(n).
# Space Complexity: O(1). Using only 5 extra variables, 2 candidate variables, 2 counter variables and a set. Counters and Candidate variables only store one item at any point and the set will at max store only 2 items,as more than that many items wont be passing the condition of there occurences found for more than n//3 times in an array of n size. Hence, all of the used variables uses constant space.
