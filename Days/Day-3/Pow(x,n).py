...
# Approach2: Better Time complexity. An even power equation for npowerm could be written as (n*n)power(m/2). Eg: 6^8 => (6*6)^4. Odd Power equation npowerm can be written as n*(npower(m-1)) Eg: 6^7=>6*(6^6). If we have 6^7 we could see its flow as: 6^7=>6*(6^6)=>6*((6*6)^3)=>6*(36*((36)^2))=>216*((36*36)^1)=>216*1296 as we encounter n=1, we'll stop going further in recursion.
class Solution:
    def pow(self, x, n):
        if (n == 1):
            return x
