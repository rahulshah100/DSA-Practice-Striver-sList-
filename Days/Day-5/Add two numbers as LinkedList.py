...
class Solution:
    def addTwoNumbers(self, l1: Optional[ListNode], l2: Optional[ListNode]) -> Optional[ListNode]:
        ...
        while l1 or l2 or carry:
            ...
            carry=sum//10
            sum%=10
            nextNode=ListNode(sum)
            summed.next=nextNode
            summed=summed.next
        print(summed)
        return dummy.next
# TC: O(max(m,n)) Explanation: To sum up all the integers from both given linked list, we'll have to go for as many iterations as there is in the length of longer number.
# SC: O(max(m,n)) Explanation: If longer list is min 10 integers, to store a new number holding summation of two given list, we'll have to store around 10 or 11 nodes only.