    ...
    def maxSubArray(self, nums: List[int]) -> int:
        ...
        for i in range(len(nums)):
            ...
            for j in range(i+1, len(nums)):
                ...
                if prevSum>maxSum:
                    maxSum=prevSum
            if nums[i]>maxSum: #in case where there is a solo array number which is the biggest sum then that endcase is captured here
                maxSum=nums[i]
        return maxSum
...
# Steps:
"""Def kadane(A):
        max_current = max_global=nums[0] #NOTE: As these are integer variable i.e. basic data-types we can declare them as such and not necessarily max_current, max_global = 0,0. This is because basic data types are never stored in referrenced manner, unlike derived datatypes such as arrays and dictionary that we'll see going forward how in derived datatype's case we need to make a cope.deepcopy() 
