...
# Approach 3: As we can see the sub-items of all arrays, even holistically they're all sorted. In such case if we can flatten the 2D array into 1D then we can use BinarySearch on it. But to flatten the array, we will have to use for loops to either traverse through all list items and store them in separate array which increases space complexity, or we can store in the same/given array, for m times, taking first item and traversing through it completely and appending all items in the same array and then deleting the item; but even this will increase time complexity by O(m*n). Therefore, what we'll do is in Binary search instead of flattening, we'll use the 2D array by figuring how to pass the appropriate indexes for the given 2D array.
class Solution:
    def searchMatrix(self, matrix: List[List[int]], target: int) -> bool:
        low = 0
