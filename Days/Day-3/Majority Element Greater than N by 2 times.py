...
# Time complexity: O(nlogn). Explaination: for the sorting.
...
"""class Solution:
    def majorityElement(self, nums: List[int]) -> int:
        ...
        for i in range(len(nums)):
            if nums[i] not in hashmap:  #Note: In List although the 'in', 'not in' operator takes O(n) time complexity, in case of dictionary and set if the keys are simple like numbers and not a string then the time complexity of in, not in is O(1)
                hashmap[nums[i]] = 1
            else:
                hashmap[nums[i]] += 1
...
S = Solution()
print(S.majorityElement([2, 2, 1, 1, 1, 2, 2]))
# Time Complexity: O(n). Explaination: Only one iteration through the given array would be required.
# Space Complexity: O(1). Explaination: We only use 2 extra variables which are counter and item. Both uses constant space.
