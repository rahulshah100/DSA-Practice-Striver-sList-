...
# Note: Here in constraints if 1 <= nums[i] <= n was not given but if it instead was 1<=nums[i]<n^2, then we'll use not an array but perhaps a dictionary as hashmap and in that case too the space complexity would become O(n), which otherwise could be O(n^2).
...
# Time Complexity: O(nlogn+n). Explaination: nlogn time for merge sort and then n time to traverse the sorted array.
# Space Complexity: O(1).
...
# Space Complexity: O(1). Explaination: IN total, there are only 2 variables which are used here. At any given time both of them: fast and slow will only single value. Hence we can consider there space usage as constant.