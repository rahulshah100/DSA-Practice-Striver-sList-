...
class Solution:
    def solve(self, A, B):
        dict = {0: True} #This is base condition or prerequisite, or we'd have to include a one more if (not elif, but if) condition where xor==B. It's like saying 9-0==9 that 9 is sum uptill 4th index and we are asked as target total of 9, and we start looking in lookup table if we had a 0 totalling entry.
        xor = ct = 0
        for i in range(len(A)):
            xor ^= A[i]
            if xor ^ B in dict:
            ...
            if xor in dict: #This is not in else condition in conjunction to above if because even when we find xor^B in dict, we'll want to store xor found at this point in our lookup table
                dict[xor] += 1
            else:
                dict[xor] = 1
        return ct
