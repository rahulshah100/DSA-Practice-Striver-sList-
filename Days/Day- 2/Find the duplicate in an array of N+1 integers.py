...
# nums.length == n + 1
# 1 <= nums[i] <= n
# All the integers in nums appear only once except for precisely one integer which appears twice or more times.
# -------------------------------------------------------------------------------------------------------------------------------------------
...
# Tortoise hayer algorithm: For the given constraints that an array is of size n+1 and the possible values at each index are from 1 to n, we can safely say that at-least one element has to repeat, because say if array is of size 6 then all possible value could be just 1,2,3,4,5 & so to make up for 6 items one value would've to be repeated. In this approach we will use the concept of building a linked list. By starting from the value of elem at 0th index, we'll start the traversal by moving to that index and checking its value, and further moving to that index. We'll take two pointer Slow(tortoise) and fast(hayer), which'll move for one and two elements respectively at each step. We'll keep traversing in the array until fast and slow pointer meets. As that happens, we'll shift fast pointer to the starting and will now move slow and fast pointer by one position each, untill they coincide again. This point where they coincide will be the repeating element.
...
# Time Complexity: O(n). Explanation: May be in first while it could take 2 times the n iterations i.e.twice after traversing the loop/array, our pointers meet. So we can represent the first while loop's time complexity as ln where l could be a constant. The second while is executed after completion of first one. The second one will at max take n iterations. So in total time complexity could be generalized and represented in terms of O(n).
# Space Complexity: O(1). Explanation: In total, there are only 2 variables which are used here. At any given time both of them: fast and slow will only hold single value. Hence, we can consider there space usage as constant.
