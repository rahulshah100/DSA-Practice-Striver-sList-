...
"""class Solution:
    def maxSubArray(self, nums: List[int]) -> int:
        maxContigousSum = -1000000000000000000
        if len(nums) == 1:
            maxContigousSum = nums[0]
        else:
            for i in range(len(nums) - 1):
                maxContigousSumForCurrentItem = nums[i]
                if maxContigousSum < maxContigousSumForCurrentItem:
                    maxContigousSum = maxContigousSumForCurrentItem
                for j in range(i + 1, len(nums)):
                    maxContigousSumForCurrentItem += nums[j]
                    if maxContigousSum < maxContigousSumForCurrentItem:
                        maxContigousSum = maxContigousSumForCurrentItem
        print(maxContigousSum)
...
# Space Complexity: O(1). We are only using 2 variables maxContiugousSum and maxContigousSumForCurrentItem. Both of these variables are using constant space.
