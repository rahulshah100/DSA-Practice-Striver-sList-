...
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# Approach1:go through all array items and further all subitems to compare if item matches.
...
# Approach2: as array items are sorted arrays, we can traverse all arrray items in a for loop, one by one. On each item, we can use Binary search to look amongst subarrayItems.
"""class Solution:
    def searchMatrix(self, matrix: List[List[int]], target: int) -> bool:
        for i in range(len(matrix)):
            found = BinSearch(matrix[i], target)
            if found == True:
                return True
        else:
            return False
...
def BinSearch(matrix, target):
    ...
    if high >= low:
        ...
        if matrix[mid] == target:
            ...
            return BinSearch(matrix[mid + 1:], target)
        else:
